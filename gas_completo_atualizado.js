...
function getColunasCartas(cab) {
  ...
  for (var i=0;i<cab.length;i++){
    ...
    if(h==="GERÊNCIA"||h==="GERENCIA"||h==="CARMS") c.gerencia=i;
...
function doPost(e) {
  ...
  try {
    ...
    var abaCartas    = getAbaCartas(sheetDb, sheets);
    var abasProcessos = [sheets[0], sheets[1], abaExternos, abaAutos, abaCartas];
      ...
      for(var s=0;s<abasProcessos.length;s++){
        ...
        for(var i=1;i<vals.length;i++){
          if(String(vals[i][colNup]).trim()===String(nupB).trim()){
            ...
            if(decisao==="APROVADO") {
              var name = aba.getName().trim().toUpperCase();
              var val = (name === "AUTOS" || name === "EXTERNOS") ? "FAZER DESPACHO" : "FAZER CI";
      ...
      if(abaAutos){var vals=abaAutos.getDataRange().getValues();
        if(vals.length>0){var cab=vals[0];var novaLinha=new Array(cab.length).fill("");
          var mapa={"NUP":data.nup,"REQUERENTE":data.requerente,"AUTO DE INFRAÇÃO":data.auto_infracao,
            ...
            "DATA STATUS ATUAL":dataAtual,
            "DATA DISTRIBUICAO":(data.tecnico && data.tecnico !== "S/T" && data.tecnico !== "-" && data.tecnico !== "Não atribuído" && String(data.tecnico).trim() !== "") ? dataAtual : ""};
          for(var c=0;c<cab.length;c++){var cn=String(cab[c]).trim().toUpperCase();if(mapa[cn]!==undefined)novaLinha[c]=mapa[cn];}
          abaAutos.appendRow(novaLinha);
          registrarTramitacao(data.nup, "CADASTRO", data.status_atual, data.setor, data.tecnico, data.username || "sistema", "Auto de Infração cadastrado no sistema.");
          SpreadsheetApp.flush();}}
    ...
    if (data.acao === "avaliar_resposta_carta") {
      ...
      for(var i=1;i<vals.length;i++){
        if(String(vals[i][cols.nup]).trim()===String(data.nup).trim()){
          ...
          if(cols.status>=0){var ns=decisao==="APROVADO"?"FAZER DESPACHO":"AGUARDANDO MANIFESTAÇÃO TÉCNICA";
    ...
    if (data.acao === "alterar_status_manual_generico") {
      ...
      if (aba) {
        ...
        if (vals.length > 0) {
          ...
          for (var c = 0; c < cab.length; c++) {
            ...
            if (hl === "STATUS ATUAL" || hl === "STATUS" || hl === "SITUAÇÃO" || hl === "SITUAÇÃO ATUAL") colStatus = c;
          ...
          if (colNup !== -1 && colStatus !== -1) {
            for (var i = 1; i < vals.length; i++) {
              if (String(vals[i][colNup]).trim() === String(nupB).trim()) {
                ...
                if (abaTramitacao) {
                  abaTramitacao.appendRow([dataAtual, nupB, "ALTERACAO_MANUAL_STATUS", novoStatus, setorOriginal, tecnicoOriginal, operador, "Status alterado manualmente pela Diretoria (" + tipoAba + ")"]);
                }
                atualizado = true; break;
              }
            }
      ...
      return ContentService.createTextOutput(JSON.stringify({status:"error",message:"NUP não encontrado"})).setMimeType(ContentService.MimeType.JSON);
    }

    if (data.acao === "alterar_status_manual_auto") {
      var nupB = data.nup, novoStatus = data.novoStatus, operador = data.username, atualizado = false;
      if (abaAutos) {
        var vals = abaAutos.getDataRange().getValues();
        if (vals.length > 0) {
          var cab = vals[0], colNup = -1, colStatus = -1, colDataStatus = -1;
          for (var c = 0; c < cab.length; c++) {
            var hl = String(cab[c]).trim().toUpperCase();
            if (hl === "NUP") colNup = c;
            if (hl === "STATUS ATUAL" || hl === "STATUS" || hl === "SITUAÇÃO") colStatus = c;
            if (hl === "DATA STATUS ATUAL" || hl === "DATA_STATUS_ATUAL") colDataStatus = c;
          }
          if (colNup !== -1 && colStatus !== -1) {
            for (var i = 1; i < vals.length; i++) {
              if (String(vals[i][colNup]).trim() === String(nupB).trim()) {
                abaAutos.getRange(i + 1, colStatus + 1).setValue(novoStatus);
                if (colDataStatus !== -1) {
                  abaAutos.getRange(i + 1, colDataStatus + 1).setValue(dataAtual);
                }
                
                // Obter dados adicionais para registrar no histórico simplificado
                var colTec = -1, colSetor = -1;
                for (var c = 0; c < cab.length; c++) {
                  var hl2 = String(cab[c]).trim().toUpperCase();
                  if (hl2 === "TÉCNICO/ADMIN" || hl2 === "TECNICO/ADMIN" || hl2 === "TÉCNICO" || hl2 === "TECNICO") colTec = c;
                  if (hl2 === "SETOR" || hl2 === "GERÊNCIA" || hl2 === "GERENCIA") colSetor = c;
                }
                var tecnicoOriginal = colTec !== -1 ? vals[i][colTec] : "S/T";
                var setorOriginal = colSetor !== -1 ? vals[i][colSetor] : "S/G";
                
                // Registra na aba de tramitação simplificada
                if (abaTramitacao) {
                  abaTramitacao.appendRow([
                    dataAtual, 
                    nupB, 
                    "ALTERACAO_MANUAL_STATUS", 
                    novoStatus, 
                    setorOriginal, 
                    tecnicoOriginal, 
                    operador, 
                    "Status alterado manualmente pela Diretoria"
                  ]);
                }
                atualizado = true;
                break;
      ...
      return ContentService.createTextOutput(JSON.stringify({status: "error", message: "NUP " + nupB + " não encontrado nos Autos."})).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (data.acao === "atribuir_tecnico_auto") {
      var nupB=data.nup,tecnico=data.tecnico,atualizado=false,setorAuto="",usuarioLogado=data.username||"sistema";
      if(abaAutos){var vals=abaAutos.getDataRange().getValues();
          ...
          if(colNup!==-1&&colTec!==-1){
            for(var i=1;i<vals.length;i++){
              if(String(vals[i][colNup]).trim()===String(nupB).trim()){
                ...
                if (colStatus !== -1) {
                  abaAutos.getRange(i+1,colStatus+1).setValue("AGUARDANDO MANIFESTAÇÃO TÉCNICA");
                }
                atualizado=true;break;}}}}}
