...
const newEndpoint = `
    if (data.acao === "alterar_status_manual_generico") {
      ...
      if (aba) {
        ...
        if (vals.length > 0) {
          ...
          for (var c = 0; c < cab.length; c++) {
            ...
            if (hl === "STATUS ATUAL" || hl === "STATUS" || hl === "SITUA��O") colStatus = c;
            if (hl === "DATA STATUS ATUAL" || hl === "DATA_STATUS_ATUAL" || hl === "DATA DE ENTRADA") colDataStatus = c;
            if (hl === "T�CNICO/ADMIN" || hl === "TECNICO/ADMIN" || hl === "T�CNICO" || hl === "TECNICO") colTec = c;
            if (hl === "SETOR" || hl === "GER�NCIA" || hl === "GERENCIA") colSetor = c;
          }
      ...
      return ContentService.createTextOutput(JSON.stringify({status:"error",message:"NUP n�o encontrado ou erro"})).setMimeType(ContentService.MimeType.JSON);
    }
`;
...
if (!code.includes("alterar_status_manual_generico")) {
    ...
    code = code.replace(
      'if(String(vals[i][colStatus]).toUpperCase()==="AGUARDANDO DISTRIBUI��O")\\n                aba.getRange(i+1,colStatus+1).setValue("AGUARDANDO MANIFESTA��O T�CNICA");',
      'aba.getRange(i+1,colStatus+1).setValue("AGUARDANDO MANIFESTA��O T�CNICA");'
    );
